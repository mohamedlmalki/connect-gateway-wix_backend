...
import { Server, PlayCircle, CheckCircle, XCircle, FileJson, Trash2, Search, RefreshCw, Save, PlusCircle, BarChart2, Users, MailCheck, MailX, MousePointerClick, Pencil, Download, ListChecks, Link, Link2, AlertCircle, MailMinus, Send } from "lucide-react";
...
interface Result { email: string; status: 'Success' | 'Failed'; message: string; fullResponse: object; }
...
export function HeadlessImportPage() {
  ...
  const handleImport = async () => {
    ...
    if (emailList.length === 0) {
      toast({ title: 'Validation Error', description: 'Please enter at least one valid email address.', variant: 'destructive' });
      setIsLoading(false);
      return;
    }
  ...
  const handleDeleteAllSelected = async () => {
    ...
    try {
        const response = await fetch('/api/headless-delete', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ memberIds: selectedAllMembers, siteId: selectedProject?.siteId }),
        });
        if (!response.ok) throw new Error('Failed to delete members.');
        toast({ title: "Success", description: `${selectedAllMembers.length} members deleted successfully.`});
      ...
      <div className="container mx-auto px-4 pt-24 pb-12">
        <div className="max-w-4xl mx-auto space-y-8">
            <div className="flex items-center justify-between gap-4 animate-fade-in">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Manage members and view campaign statistics.</p>
                    </div>
                </div>
            ...
            {(isSearching || searchResults.length > 0) && (
                <Card className="bg-gradient-card shadow-card border-primary/10">
                    <CardHeader className="flex flex-row justify-between items-center">
                        ...
                        <Button variant="outline" size="sm" onClick={() => exportEmailsToTxt(searchResults, 'search-results-emails')}><Download className="mr-2 h-4 w-4"/>Export Emails</Button>
                    </CardHeader>
                    <CardContent><div className="border rounded-lg overflow-hidden">
                        <Table><TableHeader><TableRow><TableHead className="w-[50px]"><Checkbox checked={searchResults.length > 0 && selectedMembers.length === searchResults.length} onCheckedChange={(checked) => { const allMemberIds = checked ? searchResults.map(m => m.id) : []; setSelectedMembers(allMemberIds); }} /></TableHead><TableHead>Name</TableHead><TableHead>Email</TableHead></TableRow></TableHeader>
                            <TableBody>{searchResults.map(member => (<TableRow key={member.id}><TableCell><Checkbox checked={selectedMembers.includes(member.id)} onCheckedChange={(checked) => { setSelectedMembers(prev => checked ? [...prev, member.id] : prev.filter(id => id !== member.id)); }} /></TableCell><TableCell>{member.profile?.nickname || 'N/A'}</TableCell><TableCell>{member.loginEmail}</TableCell></TableRow>))}</TableBody>
                        </Table>
                    </div></CardContent>
                ...
                <Card className="bg-gradient-card shadow-card border-primary/10 h-full">
                    ...
                    <CardContent>
                        <Select value={selectedProject?.siteId} onValueChange={(siteId) => { const project = headlessProjects.find(p => p.siteId === siteId); setSelectedProject(project); }}>
                            ...
                            <SelectContent>{headlessProjects.map(project => (<SelectItem key={project.siteId} value={project.siteId}>{project.projectName}</SelectItem>))}</SelectContent>
                        </Select>
                    </CardContent>
                </Card>
                <Card className="bg-gradient-card shadow-card border-primary/10 h-full">
                                ...
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className="w-[50px]">
                                            <Checkbox 
                                                checked={allMembers.length > 0 && selectedAllMembers.length === allMembers.length}
                                                onCheckedChange={(checked) => {
                                                    const allMemberIds = checked ? allMembers.map(m => m.id) : [];
                            ...
                            <AlertDialog>
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>This will permanently delete the selected {selectedAllMembers.length} member(s). This action cannot be undone.</AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                        <AlertDialogAction onClick={handleDeleteAllSelected} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Yes, Delete Members</AlertDialogAction>
                                    </AlertDialogFooter>
                                </AlertDialogContent>
                            </AlertDialog>
                        </DialogFooter>
            ...
            <Card className="bg-gradient-card shadow-card border-primary/10">
                ...
                <CardContent><Textarea value={emails} onChange={(e) => setEmails(e.target.value)} placeholder="user1@example.com
user2@example.com" className="h-48 resize-y font-mono text-sm" /></CardContent>
            </Card>

            <Card className="bg-gradient-primary text-primary-foreground shadow-glow">
                <CardContent className="p-6 flex items-center justify-between"><div><h3 className="text-xl font-bold">Ready to Import?</h3><p className="text-primary-foreground/80">Start the import job for the selected project.</p></div>
                <Button onClick={handleImport} disabled={isLoading || emailCount === 0} className="w-48 bg-white text-primary hover:bg-white/90" size="lg"><PlayCircle className="mr-2 h-5 w-5" />{isLoading ? 'Importing...' : 'Start Import'}</Button>
                </CardContent>
            </Card>

            {results.length > 0 && (<Card className="bg-gradient-card shadow-card border-primary/10"><CardHeader className="flex flex-row justify-between items-center">
                    <div>
                        <CardTitle>Import Results</CardTitle>
                        <CardDescription>The results of the bulk import process.</CardDescription>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => exportEmailsToTxt(results, 'import-results-emails')}><Download className="mr-2 h-4 w-4"/>Export Emails</Button>
                </CardHeader>
                <CardContent><Table><TableHeader><TableRow><TableHead>Email</TableHead><TableHead className="w-[120px]">Status</TableHead><TableHead>Details</TableHead><TableHead className="text-right">Full Response</TableHead></TableRow></TableHeader>
                    <TableBody>{results.map((result, index) => (<TableRow key={index}><TableCell className="font-mono text-xs">{result.email}</TableCell><TableCell>{result.status === 'Success' ? (<span className="flex items-center gap-2 text-green-500"><CheckCircle className="h-4 w-4" /> Success</span>) : (<span className="flex items-center gap-2 text-red-500"><XCircle className="h-4 w-4" /> Failed</span>)}</TableCell>
